			...
			if (!(brick.getHP() == INT32_MAX))
			{
				...
				if (brick.getHP() == 0)
				{
					...
					if (rand() % 2 == 0)
					{
						...
						Bonus bonus = Bonus(this->getPosition());
...
void Ball::updateBall(Paddle& paddle, float elapsed_time, Vector2f paddle_offset, std::vector<Brick>& BricksList,
	std::vector<Bonus>& BonusList, Game& game)
{
	FloatRect ball_bounds = this->getGlobalBounds();
